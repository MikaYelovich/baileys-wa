...
const { isURL } = require("../../src/utils/validator");
const fs = require("fs/promises");
const {
  createStickerWithExif,
} = require("../../src/utils/createStickerWithExif");
...
const makeMessagesSocket = (config) => {
    ...
    await authState.keys.transaction(async () => {
      ...
      if (isGroup || isStatus) {
        ...
        if (
          ...
          msgId =
            (_b = message.protocolMessage.key) === null || _b === void 0
              ? void 0
              : _b.id;
        ...
        if (
          ((_c = message.protocolMessage) === null || _c === void 0
            ? void 0
            : _c.type) === WAProto_1.proto.Message.ProtocolMessage.Type.REVOKE
        ) {
          msgId =
            (_d = message.protocolMessage.key) === null || _d === void 0
              ? void 0
              : _d.id;
          message = {};
        }
        ...
        const { user: meUser, device: meDevice } = (0, WABinary_1.jidDecode)(
          meId
        );
        if (!participant) {
          devices.push({ user });
        ...
        for (const { user, device } of devices) {
          ...
          const jid = (0, WABinary_1.jidEncode)(
            isMe && isLid
              ? ((_f =
                  (_e = authState.creds) === null || _e === void 0
                    ? void 0
        ...
        jid,
        {
          viewOnceMessage: {
            message: {
              ...
              interactiveMessage:
                WAProto_1.proto.Message.InteractiveMessage.create({
                  body: { text: title },
                  footer: { text: footer },
                  header,
                  nativeFlowMessage: { buttons },
  ...
  const sendSingleSelectButton = async (
    jid,
    {
      text,
      footer = "",
    ...
    try {
      ...
      if (!buttonText || typeof buttonText !== "string") {
        throw new Error(
          "Parameter `buttonText` is required and must be a string."
        );
      }

      if (!Array.isArray(sections) || sections.length === 0) {
        throw new Error("Parameter `sections` must be a non-empty array.");
      }
      ...
      // Build the header (media if provided)
      let header;
      if (thumbnail) {
        try {
          ...
          logger.warn(
            "⚠️ Failed to upload thumbnail, falling back to text header"
          );
          header = WAProto_1.proto.Message.InteractiveMessage.Header.create({
            title,
            hasMediaAttachment: false,
      ...
      const result = await relayMessage(jid, msg.message, {
        messageId: msg.key.id,
      });

      logger.info?.({ jid }, "✅ Sent single select interactive message");
      return result;
    } catch (err) {
      logger.error?.({ jid, err }, "❌ Failed to send single select button");
      throw new Error(`[sendSingleSelectButton] ${err.message}`);
    }
  };

  const sendSticker = async (jid, filePath, meta = {}) => {
    try {
      if (!fs.existsSync(filePath)) {
        throw new Error(`File not found: ${filePath}`);
      }

      const stickerBuffer = await createStickerWithExif(filePath, meta);

      // ✅ Replace with actual send function:
      await sendMessage(
        jid,
        {
          sticker: {
      ...
      throw new Error(`[sendSticker] ${err.message}`);
    }
  };
  ...
  return {
    ...
    sendButtonMessage: async (jid, text, footer, buttons, options = {}) => {
      const buttonMessage = buttons.map((btn) => ({
        buttonId: btn.id,
      ...
      return await sendMessage(jid, messageContent, options);
    },
    sleep: (ms) => new Promise((resolve) => setTimeout(resolve, ms)),
    fetchJSON: async (url, options = {}, config = {}) => {
      const {
      ...
      const fetchWithRetry = async (attempt = 1) => {
        try {
          ...
          throw new Error(
            `Failed to fetch ${url} after ${attempt} attempt(s): ${error.message}`
          );
        }
      };
    ...
    getBuffer: async function (data, options = {}) {
      ...
      if (
        ...
        return new Promise((resolve, reject) => {
          ...
          data.on("error", (err) =>
            reject(new Error(`Gagal memproses stream: ${err.message}`))
          );
        });
      }
      throw new Error(
        "Tipe input tidak didukung. Harap berikan URL, path file, Buffer, atau Stream."
      );
    },
  };
};
